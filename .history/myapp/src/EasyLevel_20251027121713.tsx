...
export default function EasyLevel() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => clearInterval(timeoutId), setShowElements(false) 10000);
  ...
  return (
    <>
      {showElements ? (
            ...
            {randomColors.map((colors) => (
              <button
                ...
                className={`${colors.bgcolor} ${
                  !colors.isSelected ? "opacity-100" : "opacity-50"} ${colors.width} ${colors.height} hover:scale-125 transition-transform ease-out`}
                key={colors.id}
              >
        ...
        <div className="text-red-800">Artiq baxmaq bitdi</div>
      )}
    </>
  );
}
