...
export default function EasyLevel() {
  ...
  const selectedColors = (selectedId: number) => {
    const found = ids.find((id) => id === selectedId); //null!=1  //1=1
    if (!found) {
       const newIds = [...ids, selectedId];     //1
    ...
    setRandomColors2((prevColors) =>
      prevColors.map((color) =>
        color.id === selectedId
          ? { ...color, isSelected: !color.isSelected } //1 isSeleceted:True   //1 isSelected:False
          : color
      )
        ...
        //isSelected true olanlari filter  eleyib goster 

      const result=randomColors2.filter(rnd2=>rnd2.isSelected) //burada isSelected true olanlar RANDOM2 ler [1] null
       const result2=setIds(ids.filter((idss,index)=>(
      idss===result[index].id) // isSelected true olan idler var [1]  [null]
    ))
