...
export default function EasyLevel() {
  ...
  const allExist: boolean = ids.every((id, index) => {
    id === randomColors2[index].id;
  });

  const handleResult = () => {
    // if (allExist) console.log("Her sey duzdu");
    // else console.log("Sehvdi nese");
    console.log("nese 12345 ")
  };
  return (
    <>
      {showElements ? (
        <>
          ...
          <div key={} className="flex items-center justify-center h-[70vh] gap-4">
        ...
        <>
          ...
          <div className="flex items-center justify-center h-4">
            <button className="bg-red-500 px-10  py-5" onClick={() => handleResult()}>Submit</button>
          </div>
        </>
      )}
    </>
