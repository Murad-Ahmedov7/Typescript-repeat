...
export default function EasyLevel() {
  ...
  const randomColors=[
  {id:2,colorName:"Red",bgcolor:"bg-red-500",width:"w-40",height:"h-40"},
  {id:1,colorName:"Green",bgcolor:"bg-green-500",width:"w-40",height:"h-40"},
      ...
      <div className="flex items-center justify-center h-[70vh] gap-4 ">
...

        {
          randomColors.map((colors,index)=>(
            <div className={`${colors.bgcolor} ${colors.width} ${colors.height}`} key={Math.floor(Math.random() * index+1) + 1}>
           <p className="flex justify-center items-center  w-full h-43  text-white text-xl">
            {colors.colorName}
           </p>
            </div>
