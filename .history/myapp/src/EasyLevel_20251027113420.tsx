      ...
      <div className="flex items-center justify-center h-[70vh] gap-4 ">
        {randomColors.map((colors) => (
          <button
            ...
            className={`${!colors.isSelected?colors.bgcolor:`${colors.bgcolor}/10`} ${colors.width} ${colors.height}  hover:scale-125    transition-transform ease-out  `}

            key={colors.id}
          >
            <p className="flex justify-center items-center  w-full h-4  text-white text-xl">
              {colors.colorName}
            </p>
          </button>
        ))}
