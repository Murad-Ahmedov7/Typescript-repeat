...
export default function EasyLevel() {
  ...
  const selectedColors = (selectedId: number) => {
    const found = ids.find((id) => id === selectedId); //1!=2
    if (!found) {
       const newIds = [...ids, selectedId];     //[1,yeni(2)]
    ...
    setRandomColors2((prevColors) =>
      prevColors.map((color) =>
        color.id === selectedId
          ? { ...color, isSelected: !color.isSelected } //[1 is selected true ,2 is selected true] random 2 de 
          : color
      )
      ...
      const result=randomColors2.filter(rnd2=>rnd2.isSelected) //burada isSelected true olanlar RANDOM2 ler
       const result2=setIds(ids.filter((idss,index)=>(
      idss===result[index].id) // isSelected true olan idler var
    ))
